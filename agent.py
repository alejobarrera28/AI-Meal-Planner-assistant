...
class MealPlanningAgent:
    ...
    def plan_meal(self, user_request: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "rag_retrieval_count": sum(
                tr.get("recipes_found", 0) for tr in tool_results
            ),
        }
    ...
    def _gather_information(
        ...
        for category in parsed_request["categories"]:
            result = self.tools.search_healthy_recipes(
                category=category, health_preference=parsed_request["health_preference"]
            )
    ...
    def _llm_reasoning(
        self, user_request: str, tool_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
