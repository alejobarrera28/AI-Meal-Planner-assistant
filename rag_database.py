...
Loads and queries the MealRec+ dataset using ONLY real data
"""
...
class MealRecRAGDatabase:
    """RAG Knowledge Base using ONLY real MealRec+ dataset files"""
    ...
    def __init__(self, data_path: str = "MealRec+/MealRec+H"):
        ...
        # Course metadata not available (was in removed zip files)
    ...
    def _build_real_recipe_database(self) -> Dict[int, MealRecipe]:
        """Build recipe database using ONLY real MealRec+ data"""
        recipes = {}
        ...
        for course_idx in self.course_categories.keys():
            ...
            meal_affiliations = [meal_idx for meal_idx, courses in self.meal_course_mapping.items() if course_idx in courses]
    ...
    def get_real_data_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "course_id_range": f"{min(self.course_to_index.keys())}-{max(self.course_to_index.keys())}" if self.course_to_index else "N/A",
            "health_scores_available": len(self.course_fsa_scores) > 0 and len(self.course_who_scores) > 0
        }